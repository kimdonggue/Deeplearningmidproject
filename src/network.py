...
class Modified_AlexNet(AlexNet):
    def __init__(self):
        super(Modified_AlexNet,self).__init__(num_classes=cfg.NUM_CLASSES)

        # [TODO] Modify feature extractor part in AlexNet
        self.features = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3), #kernel_size를 기존 11에서 7로 변경함으로써 조금더 디테일한 특칭 추출에 집중함함
            nn.BatchNorm2d(64), # 학습을 더 빠르고 안정되게 만듬듬
            ...
            nn.Conv2d(256, 256, kernel_size=3, padding=1), # conv_layer를 추가 함으로써 조금더 디테일한 특칭 추출에 집중함
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
...
class SimpleClassifier(LightningModule):
    def __init__(self,
        ...
        # Network
        if model_name == 'Modified_AlexNet':
            self.model = Modified_AlexNet()
        else:
